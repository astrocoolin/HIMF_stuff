...
for mass in (7.0,):
  for ba in (4,5,6,8,):
      for inc in (10,35,65,80,):
        ...
        #print(np.unique(RADI[np.isfinite(RADI)])/R_HI5,'\n')
        ...
        for i in range(0,len(RADI)):
            ...
            if len(RADI_temp[RADI_temp < R_HI5]) <= 0: break
...
R = alldelta_r.reshape(-1,1)
Y = alldelta.reshape(-1,1)

Clusters = KMeans(n_clusters=7).fit_predict(R)
...
#plt.scatter(R[Y>0],Y[Y>0])
iqr = np.array([])
for i in np.unique(Clusters):
    med_delta = np.append(med_delta,np.median(Y[Clusters == i]))
    #print(np.median(R[Clusters == i]))
    med_deltaR= np.append(med_deltaR,np.median(R[Clusters == i]))
    top,bot = np.percentile(R[Clusters == i], [75 ,25])
    iqr = np.append(iqr,top-bot)
plt.errorbar(med_deltaR,med_delta,yerr = iqr,markersize=5,fmt='+')
plt.scatter(med_deltaR,med_delta,marker='+',s=50)
plt.scatter(R,Y,c=Clusters,marker='x',alpha=0.45)
#plt.hlines(y = med_delta,xmin = med_deltaR-0.09,xmax = med_deltaR+0.09,alpha=0.5)

print(med_deltaR,med_delta)
