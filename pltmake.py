...
for gal in (1,2,3,4,5,):
 for mass in (6.5,6.0,7.5,8.0,8.5,9.0,):
  for ba in (5,):
    ...
    chimed=np.array([])
    chi2=np.array([])
    chimed = np.sum(delta_1[np.nonzero(iqr_within)]**2./iqr_within[np.nonzero(iqr_within)])/norm
    chi2   = np.sum(delta_2[np.nonzero(iqr_within)]**2./dev_within[np.nonzero(iqr_within)])/norm
    ...
    #gx1.set_title('Residual curve (sums within DHI)\n Residual Sum $\Sigma r_i$='+str(residual_int[np.argmin(abs(RADI_med - R_HI5))])+'\nUncertainty Sum $\Sigma \delta_i$='+str(error_int[np.argmin(abs(RADI_med - R_HI5))]) )
    gx1.set_title('"Bias"='+str(Total_Bias))


    #print(RADI_med,R_HI5)
    #print(residual_int)
